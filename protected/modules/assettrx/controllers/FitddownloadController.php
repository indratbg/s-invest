...
class FitddownloadController extends AAdminController
{
    public $layout = '//layouts/admin_column3';

    public function actionIndex()
    {
        $model = new Fitddownload('FIXED INCOME TD DOWNLOAD', 'FI_TD_DOWNLOAD', '');
        $model->trx_date = date('d/m/Y');
        $model->trx_status = 'A';
        $batch_file = array();

        if (isset($_POST['Fitddownload']))
        {
            $model->attributes = $_POST['Fitddownload'];
    ...
    public function getCSV(&$model, $condition)
    {
        ...
        //buy
        $select_query_buy = "select * from FI_TD_DOWNLOAD  where update_seq=$model->vo_random_value $condition AND BUY_SELL='1' ";
        $exec_data_buy = DAO::queryAllSql($select_query_buy);
        $select_query_sell = "select * from FI_TD_DOWNLOAD  where update_seq=$model->vo_random_value $condition AND BUY_SELL='2' ";
