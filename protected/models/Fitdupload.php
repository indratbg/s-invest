...
/**
 * This is the model class for table "FI_TD_UPLOAD".
 *
 * The followings are the available columns in table 'FI_TD_UPLOAD':
 ...
 */
class Fitdupload extends AActiveRecordSP
{
	...
	public function rules()
	{
		return array(
			...
			array('price, face_value', 'application.components.validator.ANumberSwitcherValidator'),
			array('file_upload','file','types'=>'csv','wrongType'=>'File type must be *.csv','on'=>'upload'),
			array('price, face_value', 'numerical'),
			array('transaction_status', 'length', 'max'=>4),
			array('im_code, br_code', 'length', 'max'=>5),
			array('im_name, br_name', 'length', 'max'=>100),
			array('security_code, security_name', 'length', 'max'=>35),
			array('buy_sell, check_status', 'length', 'max'=>1),
			...
			array('transaction_status, trade_date, settlement_date, im_code, im_name, br_code, br_name, security_code, security_name, buy_sell, ccy, price, face_value, td_reference_no, td_reference_id, check_status, cre_dt, user_id,trade_date_date,trade_date_month,trade_date_year,settlement_date_date,settlement_date_month,settlement_date_year,cre_dt_date,cre_dt_month,cre_dt_year', 'safe', 'on'=>'search'),
		);
	}
