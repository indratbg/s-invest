 ...
 */
class Fitdcancel extends Tbondtrx
{
    public $bond_cd;
    public $buy_sell;
    public $save_flg = 'N';
    ...
    {
        $sql = "SELECT TRX_SEQ_NO,TD_REFERENCE_ID,TRADE_DATE,IM_CODE,SECURITY_CODE, DECODE(BUY_SELL,1,'BUY','SELL')BUY_SELL,
                PRICE, FACE_VALUE FROM FI_TD_DOWNLOAD 
                WHERE trade_date=to_date('$trx_date','dd/mm/yyyy')
                and (security_code = '$bond_cd' or '$bond_cd'='%')
                and (im_code = '$im_code' or '$im_code'='%')
                --and Td_Reference_Id IS NOT NULL
    ...
    public function executeSpCancel()
    {
        ...
        try
        {
            $query = "CALL  SP_CANCEL_FI_TRADE_DETAIL(TO_DATE(:P_TRX_DATE,'YYYY-MM-DD'),
                                            :P_TRX_SEQ_NO,
                                            :P_ERROR_CODE,
                                            :P_ERROR_MSG)";
